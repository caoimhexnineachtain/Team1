...
def run_level_1():
    ...
    def reset_game():
        ...
        for sprite in list(cowboy_sprites):
            sprite.kill()
        pygame.mixer.music.load(join("assets", "sounds", "desert.mp3"))
    ...
    background = pygame.image.load(join("assets", "Images", "desert_background.png.png")).convert()
    ...
    cactus_surf = pygame.image.load(join("assets", "Images", "Cactus.png")).convert_alpha()
    ...
    game_over_music = join("assets", "sounds", "lossmusic.mp3")
    ...
    while running:
        ...
        if collectable_score >= 3 and not player.is_dead:
            print("LEVEL 1 COMPLETE - RETURNING LEVEL2")
            pygame.mixer.music.stop()
            return "level2"
        ...
        if player.is_dead:
            ...
            if not cowboy_spawned:
                ...
                cowboy_spawned = True
            if cowboy_spawned and not cowboy_spoke:

             cowboy = next(iter(cowboy_sprites), None)
            if cowboy and cowboy.catch_speed == 0:
