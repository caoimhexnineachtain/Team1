...
def run_level_2():
    print("LEVEL 2 FUNCTION STARTED")
    ...
    #         self.dead_surf = pygame.image.load(join("assets", "Images", "deadplayer.png"))
    ...
    #         self.is_dead = False
    #         self.death_time = 0
    #         self.death_duration = 2000  # 1 second
    ...
    class Player(pygame.sprite.Sprite):
        ...
        def update(self, dt):
            ...
            if self.on_ground:
                ...
                # Keep the bottom of the penguin on the ground
    ...
    def collisions():
        ...
        mushroom_collision = pygame.sprite.spritecollide(
            player,
            mushroom_sprites,
            False,
        ...
        bird_collision = pygame.sprite.spritecollide(
            player,
            bird_sprites,
            False,
            pygame.sprite.collide_mask
        ...
        if mushroom_collision or bird_collision:
    ...
    def reset_game():
        ...
        player.rect = player.image.get_rect(
            midbottom=(player.start_x, player.ground_y)
        )
        ...
        for sprite in list(mushroom_sprites):
            sprite.kill()

        for sprite in list(bird_sprites):
            sprite.kill()
        ...
        # ---------------- RESET TIMERS ----------------
        pygame.time.set_timer(mushroom_event, 1400)
        pygame.time.set_timer(bird_event, 1400)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_r:
                        ...
                        #  # Start jungle music again
                        # pygame.mixer.music.load(join("assets", "sounds", "jungle.mp3"))
                        # pygame.mixer.music.set_volume(0.4)
                        # pygame.mixer.music.play(-1)
                        # game_over = False
    ...
    background = pygame.image.load(join("assets", "images", "jungle_background.png")).convert()
    ...
    mushroom_surf = pygame.image.load(join("assets", "Images", "mushroom.png")).convert_alpha()
    mushroom_surf = pygame.transform.scale(mushroom_surf, (250, 250))

    bird_surf = pygame.image.load(join("assets", "Images", "junglebird.png")).convert_alpha()
    bird_surf = pygame.transform.scale(bird_surf, (220, 220))
    ...
    bird_event = pygame.event.custom_type()
    pygame.time.set_timer(bird_event, 1400)
...
# ---------------- EVENTS ----------------
        ...
        for event in pygame.event.get():
            ...
            # DOUBLE JUMP
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    if player.jumps < player.max_jumps and not player.is_dead:
            ...
            # MUSHROOM SPAWNING
            elif event.type == mushroom_event and not player.is_dead:

                x = WINDOW_WIDTH + randint(200, 300)
                y = 550

                Mushroom(
                    mushroom_surf,
                    (x, y),
                    (all_sprites, mushroom_sprites)
                )
            elif event.type == bird_event and not player.is_dead:
            
                            x = WINDOW_WIDTH + randint(200, 900)
                            y = 400
            
                            Bird(
                                bird_surf,
                                (x, y),
                                (all_sprites, bird_sprites)
                            )
        ...
        if collectable_score >= 4 and not player.is_dead:
                    print("LEVEL 2 COMPLETE - RETURNING LEVEL3")
                    pygame.mixer.music.stop()
                    return "level3"
