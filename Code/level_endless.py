...
def run_level_endless():
    ...
    def reset_game():
        ...
        for sprite in list(alien_sprites):
            sprite.kill()
    ...
    pygame.mixer.music.load(join("assets", "sounds", "space.mp3"))
    ...
    # Coin / ice cube collection sound
    ...
    pygame.time.set_timer(alien_event, 1400)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
            elif (event.type == alien_event and not player.is_dead):
    ...
    pygame.time.set_timer(ice_cube_event, 0)
